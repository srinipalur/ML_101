...
public class AVL<T extends Comparable<? super T>> {
    ...
    public void add(T data) {
        ...
        root = recurAdd(root, data);
    }

    private AVLNode<T> recurAdd(AVLNode<T> curr, T data) {
        if(curr == null) {
        ...
        if(data.compareTo(curr.getData()) < 0)
            curr.setLeft(recurAdd(curr.getLeft(), data));
        else if(data.compareTo(curr.getData()) > 0)
            curr.setRight(recurAdd(curr.getRight(), data));
        return balance(curr);
    }
    ...
    public T remove(T data) {
        ...
        root = recurRemove(root, data, dummy);
        return dummy.getData();    
    }

    private AVLNode<T> recurRemove(AVLNode<T> curr, T data, AVLNode<T> dummy) {
        if(curr == null)
            throw new java.util.NoSuchElementException("Error: some exception was thrown");
        if(data.compareTo(curr.getData()) > 0)
            curr.setRight(recurRemove(curr.getRight(), data, dummy));
        else if(data.compareTo(curr.getData()) < 0)
            curr.setLeft(recurRemove(curr.getLeft(), data, dummy));
        else {
