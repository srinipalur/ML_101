...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        int indexToUse = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> head = table[indexToUse];
        if(head == null) {
            size++;
            table[indexToUse] = new ExternalChainingMapEntry<K, V>(key, value);
            return null;
        }
        else {
    ...
    public V remove(K key) {
        ...
        int indexToUse = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> curr = table[indexToUse];
        if (curr == null)
            throw new java.util.NoSuchElementException("Error: some exception was thrown");
        if(curr.getKey().equals(key)) {
            V valueToReturn = curr.getValue();
            table[indexToUse] = curr.getNext();
            size--;
        ...
        while(curr.getNext() != null) {
            if(curr.getNext().getKey().equals(key)) {
                size--;
                V valueToReturn = curr.getNext().getValue();
    ...
    private void resizeBackingTable(int length) {
        ...
        for (int i = 0; i < table.length; i++) {
            if(table[i] == null) //if there are no records at that index
                continue;
            ExternalChainingMapEntry<K, V> curr = table[i];
            while(curr != null) {
                ExternalChainingMapEntry<K, V> newNode = new ExternalChainingMapEntry<K, V>(curr.getKey(), curr.getValue());
                int newHash = Math.abs(newNode.getKey().hashCode() % length);
                if(newTable[newHash] == null)
                    newTable[newHash] = newNode;
                else {
                    newNode.setNext(newTable[newHash]);
                    newTable[newHash] = newNode;
                }
                curr = curr.getNext();
            }
